...
#Can also look at the full name for any varaible
...
NEOst <- read.csv(file="~/github/streamcat-tutorial/streamcat_sites.csv")
...
write.csv(NEOst,file="~/github/streamcat-tutorial/NEOst.csv",row.names=F) 
```
...
NEOst <- read.csv(file="~/github/streamcat-tutorial/NEOst.csv")
...
NEO_file <- tempfile(fileext = ".gpkg")
subset <- subset_nhdplus(comids = NEOcover$COMID,
                         output_file = NEO_file,
...
plot(sf::st_geometry(flowline), col = "blue",add=T)
plot(sf::st_geometry(flowline_nldi$UT), lwd = 1, col = "red", add = TRUE)
for(pt in 1:nrow(NEOst)) {
  plot(st_sfc(st_point(c(NEOst$Longitude[pt], NEOst$Latitude[pt])), crs = 4269), cex = 1, lwd = 2, col = "red", add = TRUE)
}
plot(sf::st_geometry(waterbody), col = rgb(0, 0, 1, alpha = 0.5), add = TRUE)

for(i in 1:nrow(NEOcover)){
  nldi_feature <- list(featureSource = "comid", 
                     featureID = NEOcover$COMID[i])
  plot_data <- plot_nhdplus(
    outlets = list(featureSource = nldi_feature, featureID = NEOcover$COMID[i]), 
    gpkg = NEO_file, overwrite = TRUE)
  plot_data
}
